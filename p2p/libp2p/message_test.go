...
func TestMessage_NilMarshallerShouldErr(t *testing.T) {
	...
	pMes := &pubsub.Message{}
	m, err := libp2p.NewMessage(pMes, nil, p2p.Broadcast, p2p.MainNetwork)
...
func TestMessage_ShouldErrBecauseOfFromField(t *testing.T) {
	...
	m, err := libp2p.NewMessage(pMes, marshalizer, p2p.Broadcast, p2p.MainNetwork)
...
func TestMessage_NilMessage(t *testing.T) {
	...
	m, err := libp2p.NewMessage(nil, marshalizer, p2p.Broadcast, p2p.MainNetwork)
