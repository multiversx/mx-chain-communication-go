...
func TestMessage_NilMarshallerShouldErr(t *testing.T) {
	...
	pMes := &pubsub.Message{}
	m, err := libp2p.NewMessage(pMes, nil, p2p.BroadcastMessage)
...
func TestMessage_ShouldErrBecauseOfFromField(t *testing.T) {
	...
	m, err := libp2p.NewMessage(pMes, marshalizer, p2p.BroadcastMessage)
...
func TestMessage_NilMessage(t *testing.T) {
	...
	m, err := libp2p.NewMessage(nil, marshalizer, p2p.BroadcastMessage)
